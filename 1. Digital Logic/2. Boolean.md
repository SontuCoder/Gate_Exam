# 🔢 Boolean Algebra & Logic Gates

---

# 🔵 Bubble Logic

## NAND Gate

> [!NOTE]
> A **NAND gate** can be represented as a **pre-bubbled OR gate** using bubble logic and De Morgan's theorem.

```text
NAND = Pre-bubbled + OR
```

$$
(A \cdot B)' = A' + B'
$$

---

## NOR Gate

> [!NOTE]
> A **NOR gate** can be represented as a **pre-bubbled AND gate** using bubble logic and De Morgan's theorem.

```text
NOR = Pre-bubbled + AND
```

$$
(A + B)' = A'B'
$$

---

# 📌 Canonical Form

> [!IMPORTANT]
> In a **Canonical Form**, each term of the Boolean expression contains **all input variables**, either in **true or complemented form**.

### Example

$$
F(A,B,C)=A'B'C'+A'BC+ABC
$$

Each term contains all three variables:

```text
A, B, C
```

### Canonical Forms

- **Canonical SOP** → Sum of Minterms
- **Canonical POS** → Product of Maxterms

---

# 📌 Standard Form

> [!NOTE]
> A Boolean expression is in **Standard Form** when **at least one term does not contain all variables**.

### Example

$$
F(A,B,C)=A'+A'C+ABC
$$

Here:

```text
A'      → Missing B and C
A'C     → Missing B
ABC     → Contains all variables
```

Therefore, this is a **Standard SOP form**, but **not a Canonical SOP form**.

---

# 📚 Boolean Laws

---

## 1️⃣ Distributive Law

$$
(A+B)(A+C)=A+BC
$$

Also:

$$
A+BC=(A+B)(A+C)
$$

---

## 2️⃣ Consensus Theorem (Redundancy)

### SOP Form

$$
AB+A'C+BC=AB+A'C
$$

Here:

```text
BC = Consensus Term
```

So, `BC` is redundant and can be removed.

### POS Form

$$
(A+B)(A'+C)(B+C)=(A+B)(A'+C)
$$

Here:

```text
(B+C) = Redundant / Consensus Term
```

---

## 3️⃣ Absorption Law

$$
A+AB=A
$$

$$
A(A+B)=A
$$

### Memory Trick

```text
A + AB = A

A(A + B) = A
```

> [!TIP]
> The simpler variable `A` **absorbs** the larger expression containing `A`.

---

## 4️⃣ De Morgan's Laws

### Complement of OR

$$
(A+B+C)'=A'B'C'
$$

### Complement of AND

$$
(ABC)'=A'+B'+C'
$$

### Easy Rule

```text
Complement everything

Change OR ↔ AND
```

---

# 🔄 Duality Principle

> [!IMPORTANT]
> **Duality** is often used for the indirect proof of Boolean expressions.

To obtain the **dual** of an expression:

```text
AND (·) ↔ OR (+)

0 ↔ 1
```

> **Variables and their complements remain unchanged.**

### Example

$$
F=AB+CD
$$

Its dual:

$$
F_d=(A+B)(C+D)
$$

---

# 🔄 Complement of a Boolean Expression

To find the complement:

```text
AND ↔ OR

0 ↔ 1
```

and replace each variable with its complement.

### Example

$$
F=AB+CD
$$

Then:

$$
F'=(AB+CD)'
$$

Using De Morgan's Law:

$$
F'=(A'+B')(C'+D')
$$

---

# 🧠 Duality vs Complementation

| Operation | AND ↔ OR | 0 ↔ 1 | Variables Complemented? |
|---|:---:|:---:|:---:|
| **Dual** | ✅ | ✅ | ❌ |
| **Complement** | ✅ | ✅ | ✅ |

### Example

$$
F=AB+CD
$$

```text
Dual:
Fd = (A+B)(C+D)

Complement:
F' = (A'+B')(C'+D')
```

---

# 🔁 Dual Effect

| Original | Dual |
|---|---|
| `1` | `0` |
| `0` | `1` |
| `+ve` | `-ve` |
| `-ve` | `+ve` |
| `·` | `+` |
| `+` | `·` |
| AND | OR |
| OR | AND |
| NAND | NOR |
| NOR | NAND |
| XOR | XNOR |
| XNOR | XOR |
| NOT | NOT |
| Buffer | Buffer |

> [!NOTE]
> For **Duality**, inversion/complement symbols on variables remain unchanged.

---

# ⭐ Self-Dual Function

> [!IMPORTANT]
> A Boolean function is called **Self-Dual** if:

$$
F=F_d
$$

That means the function is equal to its own dual.

---

## 📌 Properties of Self-Dual Functions

### 1. Equal Number of Minterms and Maxterms

For a Boolean function with `n` variables:

```text
Number of input combinations = 2ⁿ
```

For a self-dual function:

$$
\text{No. of Minterms}=\text{No. of Maxterms}=2^{n-1}
$$

---

### 2. Total Number of Self-Dual Boolean Functions

For `n` variables:

$$
2^{2^{n-1}}
$$

### Comparison

| Variables | Total Boolean Functions | Self-Dual Functions |
|:---:|:---:|:---:|
| 1 | $2^{2^1}=4$ | $2^{2^0}=2$ |
| 2 | $2^{2^2}=16$ | $2^{2^1}=4$ |
| 3 | $2^{2^3}=256$ | $2^{2^2}=16$ |

---

# 🔢 Number of Boolean Functions with r Minterms

For `n` variables:

```text
Total possible input combinations = 2ⁿ
```

If the function contains exactly `r` minterms:

$$
{2^n \choose r}
=
\frac{(2^n)!}{r!((2^n-r)!)}
$$

### Meaning

We select `r` output combinations from the total `2ⁿ` possible input combinations.

---

# 🔧 Minimum NAND and NOR Gates

> [!IMPORTANT]
> The following formulas are for **2-input NAND and NOR gates**.

Let:

```text
n = Total number of variables
K = Total number of complemented variables
```

---

## 📌 Case 1: Product Term Only

When all variables are connected using **AND (·)**.

### Example

$$
F=A\cdot B\cdot \overline{C}\cdot D
$$

### Number of 2-Input Gates Required

| Implementation | Formula |
|---|---|
| **NAND Gate** | $(2n-2)+K$ |
| **NOR Gate** | $(3n-3)-K$ |

$$
\text{NAND Gates}=(2n-2)+K
$$

$$
\text{NOR Gates}=(3n-3)-K
$$

---

## 📌 Case 2: Sum Term Only

When all variables are connected using **OR (+)**.

### Example

$$
F=A+B+\overline{C}+\overline{D}
$$

### Number of 2-Input Gates Required

| Implementation | Formula |
|---|---|
| **NAND Gate** | $(3n-3)-K$ |
| **NOR Gate** | $(2n-2)+K$ |

$$
\text{NAND Gates}=(3n-3)-K
$$

$$
\text{NOR Gates}=(2n-2)+K
$$

---

# 🧠 Quick Comparison

| Expression Type | NAND Gates | NOR Gates |
|---|---|---|
| **Product / AND Form** | $(2n-2)+K$ | $(3n-3)-K$ |
| **Sum / OR Form** | $(3n-3)-K$ | $(2n-2)+K$ |

### Easy Memory Trick

```text
Product (AND form)

NAND → (2n - 2) + K
NOR  → (3n - 3) - K


Sum (OR form)

NAND → (3n - 3) - K
NOR  → (2n - 2) + K
```

---

# 🔷 SOP Form → NAND-NAND Implementation

> [!IMPORTANT]
> Whenever the minimum number of **NAND gates** is required, write or simplify the function in **SOP (Sum of Products) form**.

```text
SOP
 ↓
AND-OR Implementation
 ↓
NAND-NAND Implementation
```

### Example

$$
F=AB+CD
$$

Using NAND gates:

$$
F=((AB)'(CD)')'
$$

Therefore:

```text
SOP → AND-OR → NAND-NAND
```

> [!TIP]
> **Minimum NAND implementation → Use SOP form.**

---

# 🔷 POS Form → NOR-NOR Implementation

> [!IMPORTANT]
> Whenever the minimum number of **NOR gates** is required, write or simplify the function in **POS (Product of Sums) form**.

```text
POS
 ↓
OR-AND Implementation
 ↓
NOR-NOR Implementation
```

### Example

$$
F=(A+B)(C+D)
$$

Using NOR gates:

$$
F=((A+B)' + (C+D)')'
$$

Therefore:

```text
POS → OR-AND → NOR-NOR
```

> [!TIP]
> **Minimum NOR implementation → Use POS form.**

---

# 🎯 GATE CSE Quick Revision

> [!SUCCESS]
>
> ### Canonical Form
> - Every term contains **all variables**
> - SOP → Minterms
> - POS → Maxterms
>
> ### Standard Form
> - At least one term does **not contain all variables**
>
> ### Consensus Theorem
>
> $$
> AB+A'C+BC=AB+A'C
> $$
>
> ### Absorption
>
> $$
> A+AB=A
> $$
>
> $$
> A(A+B)=A
> $$
>
> ### De Morgan's Law
>
> $$
> (A+B)'=A'B'
> $$
>
> $$
> (AB)'=A'+B'
> $$
>
> ### Duality
>
> ```text
> AND ↔ OR
> 0   ↔ 1
> ```
>
> ### Self-Dual Functions
>
> $$
> F=F_d
> $$
>
> $$
> \text{Total Self-Dual Functions}=2^{2^{n-1}}
> $$
>
> ### Minimum NAND Gates
>
> ```text
> SOP → NAND-NAND
> ```
>
> ### Minimum NOR Gates
>
> ```text
> POS → NOR-NOR
> ```

---

# 🧠 Final Memory Trick

```text
Want Minimum NAND Gates?
        ↓
      Use SOP
        ↓
    NAND-NAND


Want Minimum NOR Gates?
        ↓
      Use POS
        ↓
     NOR-NOR
```